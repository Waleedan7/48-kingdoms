...
export default function NationsPage() {
  ...
  const filtered = nations.filter(n => {
    ...
    const matchFilter = filter === 'all' || n.status === filter || n.group_label === filter
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ fontFamily: 'Impact, sans-serif', fontSize: 28, color: 'var(--gold)' }}>LOADING NATIONS...</div>
    </div>
  )
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '2rem' }}>
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <p style={{ fontSize: 13, color: 'rgba(209,212,209,0.5)' }}>
            {nations.filter(n => n.status === 'available').length} available · {nations.filter(n => n.owner_id).length} drafted · {nations.length} total
          </p>
        </div>

        <div style={{ display: 'flex', gap: 8, marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <input type="text" placeholder="Search nations..." value={search}
            onChange={e => setSearch(e.target.value)}
            style={{
              background: 'var(--surface)', border: '1px solid rgba(255,255,255,0.1)',
              borderRadius: 8, padding: '8px 14px', color: '#fff',
              fontFamily: 'Montserrat, sans-serif', fontSize: 13, outline: 'none', width: 200,
            }}
          />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
          {filtered.map(n => (
            <div key={n.id} className="card" style={{
              padding: '1.2rem', transition: 'transform 0.2s, border-color 0.2s',
              borderColor: n.owner_id === profile?.id ? 'rgba(255,215,0,0.3)' : 'rgba(255,255,255,0.07)',
            }}
              onMouseEnter={e => (e.currentTarget.style.transform = 'translateY(-2px)')}
              onMouseLeave={e => (e.currentTarget.style.transform = 'translateY(0)')}
            >
              <div style={{ position: 'relative', marginBottom: '0.8rem' }}>
                <img src={n.flag_url} alt={n.name}
                  style={{ width: '100%', height: 70, objectFit: 'cover', borderRadius: 6 }} />
                <span style={{
                  position: 'absolute', top: 6, right: 6,
                  background: 'rgba(0,0,0,0.7)', color: 'rgba(209,212,209,0.7)',
                  fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 4,
                }}>Grp {n.group_label}</span>
              </div>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#fff', marginBottom: 4 }}>{n.name}</div>
              <span className={`badge status-${n.status}`} style={{ marginBottom: 8, display: 'inline-flex' }}>
                {n.status}
              </span>
              <div style={{ fontSize: 11, color: 'rgba(209,212,209,0.4)', marginBottom: 8 }}>
                {n.owner ? (
                  <span style={{ color: n.owner_id === profile?.id ? 'var(--gold)' : 'rgba(209,212,209,0.6)' }}>
                    👤 {n.owner.username}
                  </span>
                ) : <span style={{ color: 'var(--green)' }}>Available</span>}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
                {[['W', n.wins, 'var(--green)'], ['D', n.draws, 'rgba(209,212,209,0.5)'],
                  ['L', n.losses, 'var(--red)'], ['G', n.goals_scored, 'var(--gold)']].map(([label, val, color]) => (
                  <div key={label as string} style={{
                    background: 'var(--surface2)', borderRadius: 4, padding: '4px 6px',
              ...
              <div style={{ marginTop: 8, textAlign: 'center', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 8 }}>
                <span style={{ fontFamily: 'Impact, sans-serif', fontSize: 20, color: 'var(--gold)' }}>{n.total_points}</span>
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '3rem', color: 'rgba(209,212,209,0.3)', fontSize: 14 }}>
            No nations found
          </div>
        )}
      </div>
